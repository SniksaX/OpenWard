...
func (s *PeerService) GetLiveStats() ([]types.LivePeerStats, error) {
	...
	for _, p := range device.Peers {
		...
		if !p.LastHandshakeTime.IsZero() {
			...
			if time.Now().Unix()-lastHandshakeUnix < 180 {
				isOnline = true
			}
		}
		...
		stat := types.LivePeerStats{
			...
			IsOnline:      isOnline, // Send the bool to frontend
			LatencyMs:     0.0,      // Ready for future ping implementation
		}
...
func (s *PeerService) GetPeerConfigString(publicKey string) (string, error) {
	config, err := s.Repo.GetPeerConfig(publicKey)
	if err != nil {
		return "", fmt.Errorf("failed to locate config in database: %v", err)
	}

	if config == "" {
		return "", fmt.Errorf("no config was stored for this peer")
	}
